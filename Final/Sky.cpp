...
{
	SetupRenderStates();
	CreateDDSTextureFromFile(device.Get(), cubeMapFromDDS, 0, skyboxSRV.GetAddressOf());
}
	...
	device(device)
{
	...
	skyboxSRV = CreateCubemap(r, l, u, d, f, b);
}

Sky::~Sky() {}

Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> Sky::GetTexture() { return skyboxSRV; }

void Sky::SetupRenderStates() {
	D3D11_RASTERIZER_DESC rastDesc = {};
	rastDesc.CullMode = D3D11_CULL_FRONT;
	rastDesc.FillMode = D3D11_FILL_SOLID;
	rastDesc.DepthClipEnable = true;
	device->CreateRasterizerState(&rastDesc, skyboxRasterizer.GetAddressOf());

	D3D11_DEPTH_STENCIL_DESC depthDesc = {};
	depthDesc.DepthEnable = true;
	depthDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
	depthDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	device->CreateDepthStencilState(&depthDesc, skyboxDepth.GetAddressOf());
}
...
void Sky::Draw(std::shared_ptr<Camera> camera) {
	context->RSSetState(skyboxRasterizer.Get());
	context->OMSetDepthStencilState(skyboxDepth.Get(), 0);
	...
	skyboxPS->SetSamplerState("Sampler", samplerOptions);

	skyboxMesh->Draw(context);
